...
from models import MODE_ADD, Ingest  # noqa: E402
...
def build_crawler_db(
    ...
    excludes は crawler の --exclude 相当（`.akasyx/` のようなディレクトリ名）。
    """
...
def ingest_row(session, archive):
    row = Ingest(mode=MODE_ADD, archive_root=archive, status="running")
    session.add(row)
    session.commit()
    return row
